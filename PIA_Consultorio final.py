...
''' La función elegir opcion sirve para validar que el dato que 
introduzca el usuario corresponda a una opción válida'''
...
''' La función mostrar menu da el formato de impresión a los menus que se encuentran en el programa '''
...
''' La función crear tabla paciente almacena la consulta con la que se crea dicha tabla '''
...
''' La función crear tabla citas almacena la consulta con la que se crea dicha tabla '''
...
''' La función validar nombre se usa en registrar pacientes, 
valida que no se omita el nombre y que no contenga números '''
...
''' La función registro pacientes sirve para registrar a los pacientes y guardarlos en la tabla pacientes '''
...
''' Esta función contiene el menú para redirigirte a la opción que desees del menú de citas '''
...
''' La función crear cita sirve para programar citas para los pacientes,
 aquí el listado se muestra por orden alfabetico'''
def crear_cita():
  while True:
    try:
      with sqlite3.connect('Consultorio.db') as conn:
...
'''La función realizar citas sirve para llevar a cabo las citas previamente programadas '''
def realizar_citas():
  while True:
...
'''La función cancelar cita menú muestra el menu de los criterios para buscar la cita a cancelar y
 redirige a la función que sea necesaria '''
...
'''Esta función sirve para eliminar una citas que coincida con la fecha buscada '''
...
'''Esta función sirve para eliminar una cita que coincida con el paciente buscado '''
...
'''Esta función sirve para generar los reportes de citas, pacientes y demograficos, 
redirige a las funciones que sean necesarias '''
...
'''La funcion menu periodo paciente NO redirige a otras fuciones, 
aquí se hace lo necesario para obtener los reportes de citas por periodo y por paciente'''
...
'''La funcion menu listado busqueda clave apellido NO redirige a otras fuciones, 
aquí se hace lo necesario para obtener los reportes de pacientes'''
...
'''La funcion estadisticos demograficos NO redirige a otras fuciones, 
aquí se hace lo necesario para obtener los reportes estadisticos de peso, estatura y valores de presión
filtrando por edad y/o por sexo'''
...
'''Este while True es el código principal del programa, sirve para crear las tablas 
y cargar los datos previos en caso de existir, además, muestra el primer menú'''
while True:
  try:
    with sqlite3.connect('Consultorio.db') as conn:
      cursor = conn.cursor()
